...
class DatasetSplit(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return image, label
class Client(object):
    def __init__(self, net_client_model, idx, lr, device, idxs=None, idxs_test=None):
        self.idx = idx
        self.device = device
        self.lr = lr
        self.local_ep = 1
...
def FedAvgM(w_glob, w_locals, server_momentum, server_lr, beta):
    w_avg=copy.deepcopy(w_locals[0])
    for k in w_avg.keys():
        for i in range(1,len(w_locals)): w_avg[k]+=w_locals[i][k]
        w_avg[k]=torch.div(w_avg[k],len(w_locals))
    delta_w={k:w_avg[k]-w_glob[k] for k in w_glob.keys()}
    new_momentum={k:beta*server_momentum[k]+delta_w[k] for k in w_glob.keys()}
...
def train_server(fx_client, y, l_epoch_count, l_epoch, idx, len_batch, optimizer):
    ...
    if count1 == len_batch:
        ...
        if len(idx_collect)==num_users:
            fed_check=True
            w_glob_server, server_momentum_server = FedAvgM(net_glob_server.state_dict(), w_locals_server, server_momentum_server, server_lr, server_momentum_beta)
...
for num_users in client_counts:
    ...
    server_momentum_client = {k: torch.zeros_like(v) for k, v in net_glob_client.state_dict().items()}
    server_momentum_server = {k: torch.zeros_like(v) for k, v in net_glob_server.state_dict().items()}
...
for iter_epoch in range(epochs):
        ...
        for idx in idxs_users:
            local = Client(net_glob_client, idx, lr, device, idxs=dict_users[idx], idxs_test=dict_users_test[idx])
        ...
        w_glob_client, server_momentum_client = FedAvgM(
                                                                w_glob_client,
                                                                w_locals_client,
                                                                server_momentum_client,
                                                                server_lr,
